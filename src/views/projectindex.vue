...
const getDescription = (index: string) => {
  switch (index) {
    ...
    case "3":
      return "在当前金融科技快速发展的背景下，数据可视化在金融领域扮演着越来越重要的角色。传统的数据展示方式往往存在数据量大、更新慢、展示不直观等问题，难以满足现代金融分析的需求。同时，随着前端技术的发展，特别是 Vue 3、TypeScript 等现代化工具的成熟，为构建高性能的数据可视化平台提供了技术基础。本项目正是在这样的背景下，结合金融领域的实际需求和现代前端技术的优势，打造了一个功能完善、性能优异的数据可视化平台。项目不仅关注实际应用，还注重技术的教学价值，通过实验模块帮助开发者掌握相关技术，体现了技术创新和知识传播的双重价值。";
    case "4":
