...
watch(() => uploadedData?.value, (newVal) => {
  if (newVal) {
    state.list = newVal.map(item => ({
      gatewayno: item.leftnumone || '--',
      createTime: item.leftnumtwo || '--',
      provinceName: item.leftnumthree || '--',
    }));
  }
}, { deep: true });
      ...
      <ul class="left_boottom">
        <li class="left_boottom_item" v-for="(item, i) in state.list" :key="i">
          ...
          <div class="inner_right">
            ...
            <div class="flex">
              <div class="info">
                <span class="labels">期末现金、等价物余额：</span>
                <span class="text-content">{{ item.gatewayno }}</span>
              </div>
              <div class="info">
                <span class="labels">期初现金、等价物余额：</span>
                <span class="text-content warning" style="font-size: 12px">{{ item.createTime }}</span>
              </div>
            </div>
            <div class="info addresswrap">
              <span class="labels">现金及等价物净增加额：</span>
              <span class="text-content zhuyao doudong wangguan" style="font-size: 12px">{{ item.provinceName }}</span>
            </div>
          </div>
        </li>
      </ul>
